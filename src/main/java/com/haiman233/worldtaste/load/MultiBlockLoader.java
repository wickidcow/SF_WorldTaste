...
public final class MultiBlockLoader {
        ...
        for (String id : y.getKeys(false)) {
            ...
            try {
                ...
                if (work < 1 || work > 9 || structure[work - 1] == null) {
                    WT.log(id + ": invalid work slot (work=" + work + ", must be 1..9 and the corresponding structure slot must be non-empty); skipping");
                    skip++;
                    continue;
                }
                ...
                WT.log("mb_machines.yml " + id + " registration failed: " + e);
                skip++;
            }
        }
        WT.plugin.getLogger().info("mb_machines.yml: registered " + ok + ", skipped " + skip);
    }
    ...
    static Map<ItemStack[], ItemStack> readMbRecipes(ConfigurationSection recipesSec) {
        ...
        for (String name : recipesSec.getKeys(false)) {
            ...
            try {
                ...
                WT.log("Multiblock recipe " + name + " parse failed: " + e);
            }
        }
        return out;
    }
    ...
    private static SoundEffect parseSound(String name) {
        ...
        try {
            ...
            WT.log("Unknown SoundEffect: " + name + " (sound ignored)");
            return null;
        }
    }
}
